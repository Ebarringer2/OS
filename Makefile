x86_64_asm_source_files := $(shell find src/x86_64 -name *.asm) #holds list of all asm src files
...
$(x86_64_asm_object_files): build/x86_64/%.o : src/x86_64%.asm
  mkdir -p $(dir $@) && \
  nasm -f elf64 $(patsubst build/x86_64/%.o, src/x86_64%.asm, $@) -o $@ #input for nasm command on build
